...
import { lawyerBarVerificationApi, type LawyerBarVerification } from '~/composables/api/admin.api'
...
// State
const verifications = ref<LawyerBarVerification[]>([])
...
const selectedVerification = ref<LawyerBarVerification | null>(null)
...
// Table columns
const columns: ColumnDef<LawyerBarVerification>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return genderMap[row.original.gender] || row.original.gender
    },
  },
  {
    accessorKey: 'status',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      if (row.original.license_photo?.url) {
        return h('a', {
          href: row.original.license_photo.url,
          target: '_blank',
...
const fetchVerifications = async () => {
  ...
  try {
    const response = await lawyerBarVerificationApi.getVerifications({
...
// Handle approve
const handleApprove = (verification: LawyerBarVerification) => {
...
// Handle reject
const handleReject = (verification: LawyerBarVerification) => {
  selectedVerification.value = verification
...
const handleDialogApprove = async () => {
  ...
  try {
    const verificationId = selectedVerification.value.id || selectedVerification.value.bar_id
    await lawyerBarVerificationApi.approveVerification(verificationId)
...
const handleDialogReject = async (reason: string) => {
  ...
  try {
    const verificationId = selectedVerification.value.id || selectedVerification.value.bar_id
    await lawyerBarVerificationApi.rejectVerification(verificationId, reason)
...
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-3xl font-bold text-gray-900">Lawyer Bar Verifications</h1>
      <p class="text-gray-500 mt-1">Review and manage lawyer bar verifications</p>
    </div>
    ...
    <div v-else class="bg-white rounded-lg shadow py-5 px-2">
      <ReusableTable
        ...
        search-placeholder="Search lawyer verifications..."
