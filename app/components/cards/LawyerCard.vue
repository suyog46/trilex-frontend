...
import type { Lawyer } from '~/composables/api/lawyers.api'
...
const formattedAddress = computed(() => {
  const addr = props.lawyer.profile?.address
  if (!addr) return 'N/A'
...
// Get lawyer ID from verification or generate placeholder
const lawyerId = computed(() => {
  return props.lawyer.verification?.id || props.lawyer.id || ''
})
</script>
...
<template>
  <div class="w-full max-w-[340px] cursor-pointer rounded-xl flex flex-col p-6 bg-white border border-primary-light-active/50 hover:border-primary-normal/30 hover:shadow-xl hover:shadow-primary-normal/5 transition-all duration-300 group">
    ...
    <div class="w-full h-48 bg-gradient-to-br from-primary-light-active to-primary-light rounded-xl mb-4 flex items-center justify-center overflow-hidden relative">
      ...
      />
      <Icon v-else icon="mdi:account-tie" class="w-16 h-16 text-primary-normal group-hover:scale-110 transition-transform duration-300" />
    </div>
    ...
    <div class="flex flex-col gap-4 flex-1">
      ...
      <div>
        ...
        <p class="text-gray-600 text-sm line-clamp-2 flex items-center gap-1">
          ...
          {{ lawyer.user?.email || 'No email provided' }}
        </p>
      </div>
      ...
      <div v-if="lawyer.profile?.services?.length" class="flex flex-wrap gap-2">
        <span 
          v-for="service in lawyer.profile.services.slice(0, 3)" 
          :key="service.id"
        ...
        <span 
          v-if="lawyer.profile.services.length > 3"
          class="px-3 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-full"
        >
          +{{ lawyer.profile.services.length - 3 }} more
        </span>
      </div>
      ...
      <div class="flex flex-col gap-3 border-t border-primary-light-active/50 pt-4 mt-auto">
        <div class="flex justify-between items-center">
          ...
          <p class="text-gray-700 font-semibold text-sm">{{ lawyer.profile?.phone_number || 'N/A' }}</p>
        </div>
        <div class="flex justify-between items-center">
          <p class="text-gray-500 font-medium text-sm flex items-center gap-1">
